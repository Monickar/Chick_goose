...
def register(request):
    ...
    #         return HttpResponse(json.dumps(response))
        ...
        verify_num = request.POST.get('verify_num')
        type = request.POST.get('type')
        ...
        if type == 'verify':
            ...
            #appkey已做更改！！!!!不可用
            appkey = 'ca16e8f017bafbf005aba138b0eb8341'   # 准备工作中的App Key，类型：str
            ...
            rzb = ssender.send_with_param(86, phone_num, 1567599, [code], sign = sign1, extend ='', ext ='')
            print(rzb)
            response = {'msg': '', 'status': False}
            response['msg'] = '验证码已发送'
            return HttpResponse(json.dumps(response))
        ...
        if len(models.User.objects.filter(uid=uid)) != 0:
            # 已被创建，返回错误
            return render(request, 'sign_up.html', {'message': '用户名已被创建'})
        
        elif code == verify_num:
            print('yes, code = verify_num')
            # 插入数据
            user = {
                'uid': uid,
                'password': pwd,
